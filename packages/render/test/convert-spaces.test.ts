...
describe('CIE_Lab', () => {
  it('D65 white is (100, 0, 0)', () => {
    ...
    expect(c.c1).toBeCloseTo(100, 1);
    expect(c.c2).toBeCloseTo(0, 1);
    expect(c.c3).toBeCloseTo(0, 1);
  });

  it('black is (0, 0, 0)', () => {
    const c = convert(make(sRGB, 0, 0, 0), CIE_Lab);
    expect(c.c1).toBeCloseTo(0, 3);
    expect(c.c2).toBeCloseTo(0, 3);
    expect(c.c3).toBeCloseTo(0, 3);
  });
